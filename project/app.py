...
def current_assignment():
    ...
    try:
        ...
        # 对登机口按字母-数字顺序排序
        flights['gates'] = sorted(flights.get('gates', []), key=lambda x: (x[0], int(x[1:]) if x[1:].isdigit() else x[1:]))
...
def reassign():
    ...
    try:
        ...
        # 对重新分配结果中的登机口按字母-数字顺序排序
        if result['status'] == 'success':
            result['assignment'] = sorted(result['assignment'], key=lambda x: (x['new_gate'][0], int(x['new_gate'][1:]) if x['new_gate'][1:].isdigit() else x['new_gate'][1:]))
