#!/usr/bin/env python3
"""
Real-Time Flight Gate Management System - Core Distribution Engine

This module implements the core algorithms and data processing functions of the flight gate assignment system. The main functions include:
1. loading and pre-processing of flight data
2. gate assignment state acquisition and conversion
3. gate reassignment algorithm based on constraint planning

The core algorithm is implemented using the CP-SAT solver from Google OR-Tools, by setting constraints (e.g., time conflicts, gate availability)
and optimization objective (minimizing the distance between flights and the original gate) to generate the optimal gate allocation scheme.
